  ...
  try {
    ...
    const rows = collections.map((c, i) => `
      <tr style="background:${i % 2 === 0 ? '#f9f9f9' : '#fff'}">
        ...
        <td style="text-align:center">
          <span style="background:${c.paymentMethod === 'UPI' ? '#E3F2FD' : (c.paymentMethod === 'Later' ? '#FBE9E7' : '#F9FBE7')};padding:2px 8px;border-radius:4px">
            ${c.paymentMethod}
          </span>
        </td>
    ...
    <head>
      ...
      <style>
        ...
        .sum-card { flex: 1; min-width: 45%; border-radius: 8px; padding: 10px; text-align: center; }
        .sum-label { font-size: 11px; color: #607D8B; text-transform: uppercase; }
        .sum-val { font-size: 18px; font-weight: bold; margin-top: 2px; }
        table { width: 100%; border-collapse: collapse; font-size: 12px; }
        th { background: #1565C0; color: #fff; padding: 10px 8px; text-align: left; }
        td { padding: 8px; border-bottom: 1px solid #ECEFF1; }
        .footer { margin-top: 20px; text-align: right; font-size: 11px; color: #90A4AE; }
      </style>
    </head>
    <body>
      ...
      <div class="summary">
        ...
        <div class="sum-card" style="background:#FFF3E0">
          <div class="sum-label">Total Later</div>
          <div class="sum-val" style="color:#E65100">₹${laterAmount}</div>
        </div>
        <div class="sum-card" style="background:#E3F2FD">
          <div class="sum-label">Paid Records</div>
          <div class="sum-val" style="color:#1565C0">${paidIds.size}</div>
        </div>
    ...
    if (await Sharing.isAvailableAsync()) {
      await Sharing.shareAsync(uri, {
        ...
        dialogTitle: `Share Cable Collection Report - ${monthName} ${year}`,
        UTI: 'com.adobe.pdf',
      });
    } else {
